...
<body>
  ...
  if (isset($_POST['Ibruto'])) {
    ...
    $spreadsheet->setData($id, $bruto, $extra, $date, $employee);
    $spreadsheet->update();
  ...
  if (isset($_GET['action']) == 'edit') {
    $data = $spreadsheet->getOne($_GET['data']);
    $_SESSION['id'] = $_GET['data'];
  }
        ...
        <select required id="InputEmployee" class="input-field  form-select" name="Iemployee">
          ...
          if ($employees->rowCount() > 0) {
            foreach ($employees as $item) {
              echo '<option' . (($item['id'] == $data['employee_id']) ? ' Selected ' : '') . ' value="' . $item['id'] . '">' . $item['f_name'] . '</option>';
            }
          }
          ?>
        </select>
